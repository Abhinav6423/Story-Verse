...
const StoryDetsPopup = ({quickViewStory , setQuickViewStory}) => {
    return (
        < div
            // 1. Darker background overlay with stronger blur
        ...
        >
            <div
                // 2. Added max-h-[90vh] to strictly prevent overflowing the screen height
                // Increased to max-w-4xl for a wider, more premium cinematic shape on desktop
                // Added rounded-3xl and a subtle glowing border
            ...
            >
                {/* Close Button - Floats nicely over the layout */}
                <button
                    onClick={() => setQuickViewStory(null)}
                    className="absolute top-4 right-4 z-50 p-2.5 bg-black/40 hover:bg-white/20 backdrop-blur-xl rounded-full text-gray-300 hover:text-white transition-all duration-300 hover:scale-110 hover:rotate-90"
                >
                    <X size={18} strokeWidth={2.5} />
                </button>

                {/* Modal Image Section */}
                {/* Fixed height on mobile (h-56) to save space, stretches on desktop (md:h-auto) */}
                <div className="w-full md:w-[45%] shrink-0 h-56 md:h-auto relative bg-[#0a0c10]">
                    ...
                    />
                    {/* Multi-directional gradient for smooth blending into the text area */}
                ...
                {/* Flex container that takes remaining space, but hides overall overflow */}
                <div className="w-full md:w-[55%] flex flex-col overflow-hidden relative">

                    {/* SCROLLABLE INNER CONTENT */}
                    {/* This div handles the scrolling for long descriptions! */}
                    ...
                    {/* This stays securely at the bottom, no matter how long the text above is */}
